...
const OCTAL_ESCAPE_MATCHER = /^([^\\]|\\([^0-7]|0[^0-7]))*\\([0-7]{1,3})/;
...
module.exports = {
    ...
    /**
     * Finds an octal escape sequence in a given node
     * @param {ASTNode} node A node to check
     * @returns {Array} An array containing the matched octal escape sequence. Null
     *     if there is no match
     */
    getOctalEscapeSequence(node) {
        ...
        const match = node.raw.match(OCTAL_ESCAPE_MATCHER);
